        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-5">
          <div>
            <h2 className="text-lg font-semibold text-white tracking-tight flex items-center gap-2">
              <Briefcase className="w-5 h-5 text-neutral-400" strokeWidth={1.5} aria-hidden />
              Workspace
            </h2>
            <p className="text-sm text-neutral-500 mt-1">Pick where uploads and integrations belong.</p>
          </div>
          <button
            type="button"
            onClick={() => setIsCreating(!isCreating)}
            className="self-start shrink-0 rounded-full border border-white/15 bg-white/[0.06] px-4 py-2 text-xs font-semibold text-white hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/25 transition-colors"
          >
            {isCreating ? 'Cancel' : (
              <>
                <Plus className="w-3.5 h-3.5 inline-block mr-1 -mt-0.5 align-middle" aria-hidden />
                New workspace
              </>
            )}
          </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {workspaces.map((ws) => (
              <button
                ...
                className={`flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 text-left outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/40 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0a0a] ${selectedWorkspaceId === ws.id
                  ? 'bg-white/[0.08] border-cyan-500/25 ring-1 ring-cyan-500/15'
                  : 'bg-white/[0.03] border-white/[0.06] hover:border-white/12 hover:bg-white/[0.06]'
                  }`}
              >
                <div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-2 text-neutral-600 hover:text-red-400 hover:bg-red-500/10 rounded-full transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400/40"
                  >
            ...
            {workspaces.length === 0 && !isCreating && (
              <div className="col-span-2 py-12 text-center border border-dashed border-white/10 rounded-2xl bg-black/20">
                <Briefcase className="w-8 h-8 text-neutral-600 mx-auto mb-3" strokeWidth={1.25} aria-hidden />
                <p className="text-sm font-medium text-neutral-400">No workspaces yet</p>
                <p className="text-xs text-neutral-600 mt-1 max-w-xs mx-auto">Create a workspace to organize uploads, sheets, and webhooks.</p>
              </div>
            )}
          </div>
        )}
